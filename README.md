...
## LLM (Gemini API) 관련 참고사항

### API 키 유무에 따른 동작 차이

| 상황 | AI 채팅 | AI 리포트 | 스크립트 분석 |
|------|---------|----------|-------------|
| `GEMINI_API_KEY` 설정됨 | Gemini Flash 응답 | Gemini Pro 월간 리포트 생성 | AI 파싱 동작 |
| `GEMINI_API_KEY` 없음 | **규칙 기반 14패턴 자동 응답** | 생성 불가 (에러 메시지 반환) | 기본값 반환 |
| API Rate Limit 초과 | **규칙 기반 자동 폴백** | "API 호출 한도 초과" 메시지 | "API 호출 한도 초과" 메시지 |

### 테스트 시 유의사항

- **Gemini 무료 티어**는 분당 요청 수 제한이 있습니다 (특히 `gemini-2.5-pro`는 분당 2~5회)
- AI 채팅은 LLM 실패 시 **규칙 기반 응답으로 자동 폴백**되므로, API 키 없이도 정상 동작합니다
- AI 리포트(`POST /api/v1/projects/{id}/reports`)는 Gemini Pro를 사용하며, API 키가 없거나 Rate Limit 초과 시 400 에러를 반환합니다
- Rate Limit 에러 발생 시 **1~2분 후 재시도**하면 정상 동작합니다
- 로컬 테스트 300+개는 LLM을 Mock 처리하므로 API 키 없이 전체 통과합니다

### 규칙 기반 응답 (LLM 없이 동작하는 기능)

AI 채팅에서 LLM 없이도 다음 14개 패턴에 대해 프로젝트 데이터 기반 응답을 제공합니다:

```
인사 / BEP / 시급 / 성장성 / 수익 / 비용 / 가격 / 시간
전략 / 요약 / 목표 / 비교 / 기본 응답
```

각 패턴은 프로젝트의 실제 데이터(건당 수익, 고정비, BEP 등)를 계산하여 구체적 수치와 함께 응답합니다.
